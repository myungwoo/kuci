...
def rent_ajax_get_content(request):
	...
	hour_range = range(1, 11) if classroom.type == 1 else range(9, 22)
	...
	for hour in hour_range:
		try:
			...
			rentinfo = None
		row = {'hour': hour, 'rentinfo': rentinfo}
		rows.append(row)

	return render_to_response('rent/content.html', RequestContext(request, {'classroom': classroom, 'rows': rows, 'valid': valid}))
...
def rent_ajax_delete_rentinfo(request):
	...
	if datetime.date.today() > rentinfo.date and not request.user.is_staff:
		raise PermissionDenied()
...
def rent_ajax_write_rentinfo(request):
	...
	if datetime.date.today() > date and not request.user.is_staff:
		raise PermissionDenied()
	...
	if rentinfo_id:
		...
		print (rentinfo.classroom.id, rentinfo.date, rentinfo.hour)
		print (classroom_id, date, hour)
